<?php

return [
    App\Providers\AppServiceProvider::class,
    App\Providers\BrandsProviders::class,
    App\Providers\ContactProvider::class,
    App\Providers\CoursesProviders::class,
    App\Providers\FProductServiceProvider::class,
    App\Providers\FileProviders::class,
    App\Providers\FrontNewsProviders::class,
    App\Providers\GalleryProviders::class,
    App\Providers\ImagesProviders::class,
    App\Providers\PortfolioCategoryProviders::class,
    App\Providers\PortfolioImageProviders::class,
    App\Providers\PortfolioImagesProvider::class,
    App\Providers\PortfoliosProviders::class,
    App\Providers\ReferencesProviders::class,
    App\Providers\ServicesProviders::class,
    App\Providers\SettingsProviders::class,
    App\Providers\SlidersProviders::class,
    App\Providers\VideoProviders::class,
];
